    ...
    <footer className="border-t border-slate-800 bg-[#060D1A] py-10">
      <div className="container-page grid gap-8 md:grid-cols-[1fr_2fr]">
        <div>
          ...
          <p className="max-w-md text-[13px] leading-7 text-slate-400">
          ...
          <p className="mt-3 text-[13px] leading-7 text-slate-400">
            للاستفسارات وطلبات الخصوصية: <ObfuscatedEmail user="privacy" domain="arbahh.com" className="underline underline-offset-2 text-slate-400 hover:text-slate-200" />
          </p>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {links.map(([label, href]) => (
            <Link className="block text-[13px] leading-[2.2] text-slate-400 underline-offset-2 hover:underline hover:text-slate-200" href={href} key={href}>
              {label}
            </Link>
          ))}
        </div>
      </div>
      <div className="container-page mt-6 border-t border-slate-800 pt-4 text-xs leading-6 text-slate-400">
